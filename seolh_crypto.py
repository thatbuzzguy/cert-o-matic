...
def set_subject_name(config_obj, common_name):
    subject_name_obj = x509.Name([
        x509.NameAttribute(NameOID.COUNTRY_NAME, config_obj.data[
                           'root_cert_config']['country_name']),
        x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, config_obj.data[
            'root_cert_config']['state_or_province']),
        x509.NameAttribute(NameOID.LOCALITY_NAME, config_obj.data[
            'root_cert_config']['city_or_locality']),
        x509.NameAttribute(NameOID.ORGANIZATION_NAME, config_obj.data[
            'root_cert_config']['organization']),
        x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, config_obj.data[
...
def set_csr(private_key_obj, subject_obj, hash_obj, backend_obj):
    csr_obj = x509.CertificateSigningRequestBuilder().subject_name(
        subject_obj).sign(private_key_obj, hash_obj, backend_obj)
    return csr_obj
...
def sign_cert(self_signed, private_key_obj, csr_obj, cert_lifetime_obj, hash_obj, config_obj, backend_obj):
    ...
    builder_obj = builder_obj.issuer_name(x509.Name(
        [x509.NameAttribute(NameOID.COMMON_NAME, config_obj.data['root_cert_config']['common_name'])]))
    ...
    builder_obj = builder_obj.add_extension(x509.BasicConstraints(
        ca=self_signed, path_length=None), critical=True, )
...
def pem_encode_private_key(private_key_obj):
    pem = private_key_obj.private_bytes(encoding=serialization.Encoding.PEM,
...
def pem_encode_public_key(private_key_obj):
    pem = private_key_obj.public_bytes(
