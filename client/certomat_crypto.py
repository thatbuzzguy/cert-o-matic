...
def set_subject_name(config_obj):
   subject_name_obj = x509.Name([
      x509.NameAttribute(NameOID.COUNTRY_NAME, config_obj.data['ca_config']['country_name']),
      x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, config_obj.data['ca_config']['state_or_province']),
      x509.NameAttribute(NameOID.LOCALITY_NAME, config_obj.data['ca_config']['city_or_locality']),
      x509.NameAttribute(NameOID.ORGANIZATION_NAME, config_obj.data['ca_config']['organization']),
      x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, config_obj.data['ca_config']['organizational_unit']),
      x509.NameAttribute(NameOID.COMMON_NAME, config_obj.data['ca_config']['common_name']),
      x509.NameAttribute(NameOID.EMAIL_ADDRESS, config_obj.data['ca_config']['email_address'])
      ,])
...
def sign_cert(self_signed, private_key_obj, csr_obj, cert_lifetime_obj, hash_obj, config_obj, backend_obj):
   ...
   temp = config_obj.serial_number
   serial_number_str = hex(temp)[2:]
   save_path = 'certificates\\'
   
   builder_obj = x509.CertificateBuilder()
   builder_obj = builder_obj.issuer_name(x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, config_obj.data['ca_config']['issuer_name'])]))
   builder_obj = builder_obj.not_valid_before(utcnow)
   builder_obj = builder_obj.not_valid_after(utcnow + cert_lifetime_obj)
   builder_obj = builder_obj.serial_number(config_obj.serial_number)
   ...
   with open(config_obj.data['ca_config']['database'], "a") as database:
...
def set_certificate_lifetime(config_obj):
   certificate_lifetime_obj = datetime.timedelta(days=config_obj.data['ca_config']['certificate_lifetime_in_days'])
...
def initalize(config_obj, backend_obj):
   ...
   with open(config_obj.data['ca_config']['private_key_file'], "wb") as f:
...
def process_request(config_obj, backend_obj):
   subject_obj = set_subject_name(config_obj)
   private_key_obj = set_private_key(config_obj, backend_obj)
   hash_obj = set_hash_name(config_obj)
   certificate_lifetime_obj = set_certificate_lifetime(config_obj)
