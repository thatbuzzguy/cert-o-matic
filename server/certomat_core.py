...
def set_hash_name(certificate_obj):
   hash_name = certificate_obj.data['ca_config']['hash_name']
   if hash_name == 'sha256':
...
def set_private_key(certificate_obj, backend_obj):
   algorithm_name = certificate_obj.data['ca_config']['algorithm_name']
...
def set_subject_name(certificate_obj):
   subject_name_obj = x509.Name([
      x509.NameAttribute(NameOID.COUNTRY_NAME, certificate_obj.data['ca_config']['country_name']),
      x509.NameAttribute(NameOID.STATE_OR_PROVINCE_NAME, certificate_obj.data['ca_config']['state_or_province']),
      x509.NameAttribute(NameOID.LOCALITY_NAME, certificate_obj.data['ca_config']['city_or_locality']),
      x509.NameAttribute(NameOID.ORGANIZATION_NAME, certificate_obj.data['ca_config']['organization']),
      x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, certificate_obj.data['ca_config']['organizational_unit']),
      x509.NameAttribute(NameOID.COMMON_NAME, certificate_obj.data['ca_config']['common_name']),
      x509.NameAttribute(NameOID.EMAIL_ADDRESS, certificate_obj.data['ca_config']['email_address'])
      ,])
...
def sign_cert(self_signed, private_key_obj, csr_obj, cert_lifetime_obj, hash_obj, certificate_obj, backend_obj):
   ...
   builder_obj = builder_obj.issuer_name(x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, certificate_obj.data['ca_config']['issuer_name'])]))
   ...
   with open(certificate_obj.data['ca_config']['database'], "a") as database:
...
def set_certificate_lifetime(certificate_obj):
   certificate_lifetime_obj = datetime.timedelta(days=certificate_obj.data['ca_config']['certificate_lifetime_in_days'])
...
def initalize(certificate_obj, backend_obj):
   ...
   issuer_name = certificate_obj.data['ca_config']['issuer_name']
   ...
   with open(certificate_obj.data['ca_config']['private_key_file'], "wb") as f:
